...
@objc(VSMVirgilCrypto) public class VirgilCrypto: NSObject {
    ...
    @objc public func decrypt(_ data: Data, with privateKey: VirgilPrivateKey) throws -> Data {
        ...
        return try cipher.decryptData(data, recipientId: privateKey.identifier, privateKey: privateKey.rawKey, keyPassword: nil)
    }
    
    @objc public func decrypt(_ stream: InputStream, to outputStream: OutputStream, with privateKey: VirgilPrivateKey) throws {
        let cipher = ChunkCipher()
        
        try cipher.decrypt(from: stream, to: outputStream, recipientId: privateKey.identifier, privateKey: privateKey.rawKey, keyPassword: nil)
    }
    ...
    @objc public func decryptThenVerify(_ data: Data, with privateKey: VirgilPrivateKey, using signerPublicKey: VirgilPublicKey) throws -> Data {
        ...
        let decryptedData = try cipher.decryptData(data, recipientId: privateKey.identifier, privateKey: privateKey.rawKey, keyPassword: nil)
